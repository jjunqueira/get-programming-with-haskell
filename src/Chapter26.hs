...
marcRecordLengthProgram :: IO ()
marcRecordLengthProgram = do
  marcData <- B.readFile "ohsu_ncnm_wscc_bibs.mrc"
  let marcRecords = allRecords marcData
  print (length marcRecords)

booksProgram :: IO ()
booksProgram = TIO.writeFile "books.html" (booksToHtml myBooks)

marcFileToHtmlProgram :: IO ()
marcFileToHtmlProgram = do
  marcData <- B.readFile "ohsu_ncnm_wscc_bibs.mrc"
